...
export const metadata: Metadata = {
  ...
  description:
    'Case study: a solo, freelance full-stack platform for a martial arts studio built on Next.js 16 and Sanity, with an env-switched dual data source that adopts the CMS as a zero-downtime feature flag.',
  openGraph: {
    type: 'article',
    title: 'Reisky — Case Study | Jon Wayne Cabusbusan',
    description:
      'An env-switched dual data source, a type-safe GROQ pipeline, and Accessibility 100 site-wide — a solo full-stack build.',
  },
};
    ...
    <>
      ...
      <main id="main-content">
        ...
        <section className="relative scroll-mt-20 py-16 md:py-24" aria-labelledby="case-study-title">
          <div className="section-shell">
            <p className="section-kicker">Case study · {typeLabel(cs.role)}</p>
            <h1
              id="case-study-title"
            ...
            <dl className="mt-7 flex flex-wrap gap-x-10 gap-y-4">
              <Meta term="Client" value={cs.client} />
              <Meta term="Role" value={cs.role} />
              <Meta term="Timeline" value={cs.period} />
            </dl>

            <p className="mt-7 max-w-3xl text-lg leading-8 text-muted-foreground">{cs.summary}</p>
        ...
        <section className="editorial-rule py-12 md:py-16" aria-label="Project at a glance">
          <div className="section-shell">
            <dl className="premium-panel grid grid-cols-2 divide-x divide-y divide-border md:grid-cols-3">
              {cs.metrics.map((metric) => (
                <div key={metric.label} className="flex flex-col gap-2 p-5 sm:p-6">
                  <dt className="font-serif text-3xl font-bold leading-none text-foreground sm:text-4xl">
                    {metric.value}
                  </dt>
                  <dd className="text-sm leading-6 text-muted-foreground">{metric.label}</dd>
                </div>
              ))}
            </dl>
          </div>
...
// "Solo Developer (Freelance)" -> "Freelance" for the kicker badge line.
function typeLabel(role: string): string {
  return role.toLowerCase().includes('freelance') ? 'Freelance' : 'Project';
}
